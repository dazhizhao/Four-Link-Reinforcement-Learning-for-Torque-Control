...
class TrainConfig:
    ...
    def load(cls, config_path: str | Path | None = None) -> "TrainConfig":
        path = (
            Path(config_path)
            if config_path
            else ROOT / "configs" / "train_link_allocation.yaml"
        )
...
def main() -> None:
    ...
    run_dir, resolved_run_name = build_run_dir(output_root, "rl_link_alloc", run_name)
    ...
    print(f"training link allocation: run={resolved_run_name} timesteps={total_timesteps} seed={seed}")
    ...
    samples_path = save_workspace_samples(
        output_path=artifacts_dir / "best_workspace_samples.npz",
    ...
    preview_path = render_workspace_preview(
        output_path=artifacts_dir / "best_workspace.png",
        points=evaluation["best_workspace_points"],
        lengths=evaluation["best_lengths"],
        representative_joint_angles=evaluation["best_representative_joint_angles"],
        xy_bounds=evaluation["best_xy_bounds"],
    ...
    video_path = export_workspace_video(
        output_path=artifacts_dir / "best_workspace.mp4",
        points=evaluation["best_workspace_points"],
        lengths=evaluation["best_lengths"],
        representative_joint_angles=evaluation["best_representative_joint_angles"],
        xy_bounds=evaluation["best_xy_bounds"],
    ...
    summary_payload = {
        ...
        "evaluation": {
            ...
            "best_lengths": evaluation["best_lengths"],
            "best_grid_shape": evaluation["best_grid_shape"],
            "best_xy_bounds": np.asarray(evaluation["best_xy_bounds"], dtype=np.float32).tolist(),
